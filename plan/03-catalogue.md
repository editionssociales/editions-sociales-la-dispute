...
| `edition` | select `editions-sociales`\|`la-dispute`, **nullable**, indexé | **contrat d'interface posé par la phase 4** : les entrées boutique-seules de septembre (~20 produits orphelins) n'ont pas de maison ; toutes les fiches migrées en ont une | site d'origine |
...
| Q3 | **Les 9 liens boutique cassés** (dérive de slug post-prévente — R2 §2.2) : corriger dans Payload (les livres redeviennent « En vente ») ou laisser en l'état ? | Avant le swap (20/07) | **Corriger les 9** (mapping par similarité de titre proposé, validation ligne à ligne avec l'équipe — 30 min) ; les 20 produits orphelins restent pour la phase commerce |
