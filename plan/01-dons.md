...
| HelloAsso | **Branche de repli documentée** (cf. Questions ouvertes Q1), pas la branche par défaut | Ne vaut que si la structure est une association ET veut des reçus fiscaux ; Stripe marche quel que soit le statut, et le compte sert aussi au commerce natif (livré, lots 1–2 — cf. `plan/04-commerce.md` — payouts unifiés) |

**Correctif d'acquis intégré (RECON R2 §2.4) — mis à jour au 11/07** : la boutique legacy encaisse via **Paybox** (« ils sont déjà sur Stripe » du devis était faux — 0 commande Stripe depuis 2018 ; confirmé par un checkout de test le 11/07, commande 7730, formulaire `PBX_*`). Le compte de la config Woo (`acct_1SQlxX…`) est un compte **mode test abandonné** (nettoyage de ses webhooks : phase 7). **MAIS le chemin critique KYC est LEVÉ** : un compte Stripe **live et opérationnel** existe — `acct_1TqsjgL6ffEZ7VRj` « Éditions sociales » (`administrer@editionssociales.fr`, FR/EUR, compte standard), **vérifié par API le 11/07 : `details_submitted`, `charges_enabled` et `payouts_enabled` = true, zéro pièce en attente** — et sa clé secrète live est déjà dans `site/.env`. Le chemin critique de la phase se réduit donc aux **pages légales** (E10b/P9) et aux **contenus 2026** (P6). « Une seule caisse » (dons + commerce natif, livré — cf. `plan/04-commerce.md` — sur ce compte) reste l'objectif.
...
|---|---|---|---|---|
| P1 | ~~Lancer l'activation live du compte Stripe~~ **✅ FAIT (constat du 11/07)** : compte live `acct_1TqsjgL6ffEZ7VRj` « Éditions sociales » opérationnel (`charges_enabled` + `payouts_enabled` = true, `requirements.currently_due` vide — vérifié par API), clé secrète live déjà dans `site/.env`. Reste : récupérer la **clé publique** (`pk_live_…`) et créer l'**endpoint webhook** (E9/E11 — le `whsec_…` naît avec l'endpoint) | Youri | fait / reste avec E9 | Ne PAS réutiliser l'ancien compte test `acct_1SQlxX…` de la config Woo (abandonné — nettoyage phase 7). Vérifier avec le client que l'IBAN de payout est bien celui de la structure |
| P2 | Accès **Dashboard** Stripe pour Youri (rôle **Developer** minimum, Admin idéalement — reçus, branding, disputes, clés test) | Client (si le compte n'a pas été créé par Youri) | dès que possible, non bloquant pour le dev | Dashboard → Settings → Team. Les clés ne transitent jamais par email |
| P3 | Clés **test** dans l'environnement de dev | Youri | 10/07 (repli : sandbox Stripe perso — le code est identique) | `site/.env.local` : `STRIPE_SECRET_KEY=sk_test_…` (mode test du compte live) + `STRIPE_WEBHOOK_SECRET=whsec_…` (celui du `stripe listen`). ⚠️ **Jamais la clé live en dev local** — elle ne vit qu'en cible `production` Vercel (P4) et dans `site/.env` (gitignoré — vérifié : `.env*` sauf `.env.example`) |
| P4 | Clés dans **Vercel** (projet `editions-sociales-la-dispute`, team `solidz`) | Youri | avec la 1re mise en ligne (E9) | ⚠️ **Piège R4** : utiliser le `VERCEL_TOKEN` **du shell** (`yourimerad`/`solidz`), PAS celui de `site/.env` (team `ldes` vide, accès refusé). `vercel env add STRIPE_SECRET_KEY production` + `STRIPE_WEBHOOK_SECRET production` (valeurs **test** d'abord : en cible `production` jusqu'à E11 **et** en cible `preview` — 2 min qui débloquent la recette commerce (déjà codé, lots 1–2 mergés — cf. `plan/04-commerce.md`) ; la clé **live**, elle, reste `production` uniquement, posée au jour J — jamais en preview/development) |
...
2. Créer l'endpoint webhook **live** : même URL, mêmes 3 événements → nouveau `whsec_live_…`. **Le jour du flip DNS** (désormais dans la fenêtre de bascule unique, 24–28/07 proposée — le runbook exact, y compris ce geste, vit dans `plan/02-mise-en-production.md` §Runbook Jour J unique, ce document ne le duplique pas) : le second endpoint webhook sur le domaine final et l'activation du « Redirect to Primary » sur l'URL `*.vercel.app` sont simultanés — redirect à ne **PAS** activer avant : les livraisons webhook sur l'URL vercel.app recevraient des 307/308 que Stripe ne suit pas. Garder les **deux** endpoints actifs ensuite.
...
|---|---|---|
| ~~KYC Stripe live pas prêt à temps~~ **ÉTEINT (11/07)** : compte live opérationnel vérifié par API (`charges_enabled`/`payouts_enabled` true, 0 pièce due). Risques résiduels : review Stripe **post-activation** possible sur les premiers encaissements (catégorie d'activité, volumes inhabituels le 15/08) ; accès Dashboard/2FA détenus par une seule personne pendant la fermeture d'août | Faible / retard de payout, pas de blocage d'encaissement | Site public avec mentions légales avant les dons réels (P9/E10b — rassure toute review) ; brancher les **alertes email Stripe** (paiements, disputes) sur une adresse lue en août ; s'assurer que Youri ET le client ont accès au Dashboard avant la fermeture (P2) |
...
- **Phase 2 (mise en production / DNS)** : `success_url`/`cancel_url` dérivées du host de la requête (`headers()`) → survivent au cutover `editionssociales.fr` → Vercel sans code ; au cutover — désormais dans la fenêtre de bascule unique (24–28/07 proposée, cf. `plan/02-mise-en-production.md` §Runbook Jour J unique, qui possède ce geste et n'est pas dupliqué ici) — **ajouter un endpoint webhook** pour le domaine final `editionssociales.fr` (garder les deux), **simultanément** à l'activation du « Redirect to Primary » sur l'URL `*.vercel.app`, qui ne doit **pas** être activé avant (les livraisons webhook sur vercel.app recevraient des 307/308 que Stripe ne suit pas). **Pages légales : la dépendance est désormais datée** — elles sont condition d'entrée d'E11, avant la fenêtre de bascule unique ; si la phase Mise en production ne les a pas livrées à J−2, le filet E10b de cette phase publie la version minimale (gabarit C90), que la phase Mise en production complète ensuite (sitemap, 301, robots). Le point « SSO previews avant démo 15/07 » appartient aussi à cette phase.
...
- **Phase commerce natif (livrée, lots 1–2 mergés le 12/07 — cf. `plan/04-commerce.md`)** : réutilise `src/lib/stripe.ts` et étend `src/app/api/stripe/webhook/route.ts` (switch sur `event.type`, discrimination par `metadata.kind` : `donation` posé dès E3, `order` pour les commandes) ; **même compte Stripe** = payouts unifiés (la promesse du devis) ; `metadata.campaign` sépare dons et ventes dans les exports et dans la jauge (la recherche filtre sur la metadata, les ventes commerce n'y entreront jamais). La bascule PSP Paybox→Stripe de la boutique est un sujet de CETTE phase-là, pas des dons — et elle bascule désormais dans la même fenêtre que les dons (plan/02), pas en septembre.
...
| **avant la fenêtre de bascule unique (24–28/07 proposée)** | **E11 passage en réel** (dès `charges_enabled` ET pages légales en ligne) ; recette réelle (don 1 € puis remboursement, R13/R14) ; le jour du flip DNS (cf. `plan/02-mise-en-production.md` §Runbook Jour J unique) : second endpoint webhook sur `editionssociales.fr` créé **simultanément** à l'activation du « Redirect to Primary » (pas avant — cf. E11.2) |
...
|---|---|---|---|
| Q1 | **Stripe direct ou HelloAsso ?** (dépend du statut : association loi 1901 → HelloAsso 0 % + reçus fiscaux auto) | **lun 13/07** (sinon le jour vendu part en Stripe et un revirement coûte un re-dev) | **Stripe** : marche quel que soit le statut, **compte live déjà opérationnel (vérifié 11/07 — zéro délai d'activation)**, une seule caisse avec le commerce natif (livré, lots 1–2), pas de « pourboire HelloAsso » imposé aux donateurs. Branche HelloAsso si choisie : boutons → formulaire HelloAsso (lien ou widget iframe), jauge via API v5 (OAuth client_credentials, fetch taggé identique), pas de webhook — même enveloppe 1 j |
