...
- `pnpm build` : hermétique (Postgres uniquement) — rejoué dans `ci.yml` après `pnpm knip`, via les secrets GitHub Actions `DATABASE_URL`/`PAYLOAD_SECRET` (posés le 2026-07-26, cf. `DEVOPS.md` § « Pipeline CI/CD » — chaque run consomme donc du transfert Neon), en plus du build de déploiement (`vercel-build` = migrate:prod + next build ; un échec laisse l'ancien déploiement en ligne). Il n'y a plus de preview Vercel sur PR : ce job « verify » est la SEULE vérification du build avant merge.
