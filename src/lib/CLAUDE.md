...
- The Book→`NouveauteBook` carousel mapping lives ONCE in `nouveaute-book.ts` (`toNouveauteBooks`) — consumed by the home page and the `?upcoming=1` catalogue view; never re-inline it.
