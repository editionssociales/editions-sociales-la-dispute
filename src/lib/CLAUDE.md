...
- Euros↔cents conversion lives ONCE in `money.ts` (`eurosToCents`/`centsToEuros`) — consumed by `cart-core`/`checkout-core`/`promo-core` (euros→cents) and `order-webhook-core` (cents→euros); `order-export.ts:roundCents` stays separate (a euros rounding, not a conversion).
