 ...
 */
/**
 * Sélection cochée (`ids`, checkboxes de la liste — l'autre volet de la
 * demande cliente) : PRIME sur les filtres de la vue, et ses erreurs sont
 * EXPLICITES (parsing pur dans `order-export.ts:parseExportOrderIds`, déjà
 * testé — ici on verrouille l'orchestration : priorité, aucune clause
 * ajoutée, 400 sur `ids` illisible).
 */
describe('exportPreparationHandler / exportComptaHandler — sélection cochée (`ids`)', () => {
  it('`ids` posé → where = ces ids exactement, les filtres de la vue ne sont PAS lus', async () => {
    const { payload, findCalls } = fakePayload({ orders: [order()] })
    const res = await exportComptaHandler(
      req(`${URL_COMPTA}?ids=12,45,109`, {
        user: { role: 'admin' },
        payload,
  ...
  it('aucun statut imposé non plus : une commande cochée sort du profil préparation quel que soit son statut', async () => {
    const { payload, findCalls } = fakePayload({ orders: [order({ status: 'shipped' })] })
    const res = await exportPreparationHandler(
      req(`${URL_PREP}?ids=42`, { user: { role: 'admin' }, payload }),
    )
    expect(res.status).toBe(200)
  ...
  it('`ids` illisible → 400 qui nomme le segment fautif, jamais de lecture Payload', async () => {
    const { payload, find } = fakePayload()
    const res = await exportComptaHandler(
      req(`${URL_COMPTA}?ids=12,abc`, { user: { role: 'admin' }, payload }),
    )
    expect(res.status).toBe(400)
  ...
  it('`ids` vide → pas une sélection : retombe sur les filtres de la vue', async () => {
    ...
    await exportComptaHandler(
      req(`${URL_COMPTA}?ids=`, { user: { role: 'admin' }, payload, query: { where } }),
    )
    expect(findCalls.find((c) => c.collection === 'orders')?.where).toEqual(where)
  })
})
